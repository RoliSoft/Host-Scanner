...
public:
	 ...
	 */
	std::unordered_set<std::string> FindVulnerability(const std::string& cve, OpSys distrib = OpSys::Ubuntu, float ver = 0.0) override;
	 ...
	 */
	std::string GetUpgradeCommand(const std::unordered_set<std::string>& pkgs, OpSys distrib = OpSys::Ubuntu, float ver = 0.0) override;
