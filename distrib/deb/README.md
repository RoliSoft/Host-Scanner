# Host Scanner DEB Distribution

This folder contain the `Dockerfile` and scripts to pull, compile and package the latest revision. The scripts here are tailored to produce a `.deb` package, which is generated with the current Debian Stable, `debian:latest` in Docker Hub.
